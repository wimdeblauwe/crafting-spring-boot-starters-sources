...
public record RequestIdProperties(
        ...
        @DefaultValue Map<String, HeaderName> pathStrategies) {

    /**
     * Header settings.
     *
     * @param name The inbound header name to read the request id from. Must be a valid
     *             HTTP header name (letters, digits, hyphens; starting with a letter).
     * @param echo When true, the same header is written on the response so the
     *             client can see which id the server assigned.
     */
    public record Header(
            @DefaultValue("X-Request-Id") HeaderName name,
